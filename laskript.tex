...
\begin{satz}[Weierstrass-Normalform]
    Sei $A\in\M_{n,n}(K).$ Dann existiert ein eindeutig bestimmtes $m\in\N$, Polynome $h_1,...,h_m\in K[t]$, dei Potenzen von irreduziblen Polynomen sind, sodass 
    $$A\approx B_{h_1,...,h_m}.$$
    $h_1,...,h_m$ sidn eindeutig bis auf die Reihenfolge bestimmt und heißen die \textbf{Weierstrassteiler} von $A$.
...
\begin{bsp}
    \begin{enumerate}[(a)]
        ...
        3 & 1 & 6
        \end{pmatrix}\in\M_{3,3}(\Q)$$
        $\implies c_1(A)=1,c_2(A)=1,c_3(A)=(t-1)(t-2)^2.$ Mit $h_!=t-1,h_2=(t-2)^2=t^2-4t+4$ ist 
        $$A\approx B_{h_1,h_2}=
