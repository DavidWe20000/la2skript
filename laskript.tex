...
\begin{bsp}
    ...
    \end{align*}
        ...
        \end{notation}
        \begin{satz}[Universelle Eigenschaft des direkten Produkts]
         ...
         \begin{enumerate}[(UP)]
                \item Für jeden $R$-Modul $N$ und jede Familie $(f_i)_{i\in I}$ von $R$-Modulhom. $f_i:N\longrightarrow M_i$ existiert genau ein $R$-Modulhom. $f:N\mapsto M $ mit $f_i=p_i\circ f$ für alle $i\in J$
                \[ 
                \begin{xy}
                    \xymatrix{
        ...
        \begin{proof}
            ...
            \begin{enumerate}[1.]
                ...
                \end{xy}
                \]
                $h=\id_M$ erfüllt das, wegen $(\ast)$ erfüllt das auch $h=\boldsymbol{\Psi}\circ\boldsymbol{\Phi}\implies \boldsymbol{\Psi}\circ\boldsymbol{\Phi}=\id_M.$ Analog: $\boldsymbol{\Phi}\circ\boldsymbol{\Psi}=id_{M'}.$
            \end{enumerate}
        \end{proof}
        \begin{definition}[direktes Produkt über univ. Eigenschaft]
        ...
        \end{definition}
        \begin{satz}[Universelle Eigenschaft der direkten Summe]
            Seien $(M_i)_{i\in I}$ eine Familie von $R$-Moduln, $M=\bigoplus\limits_{i\in I}M_i,$
            $$q_i:M_i\longrightarrow M,(q_i(m))_j=\left\{\begin{array}{cc}
                m & i=j\\
                0 & \text{sonst}
            \end{array}
                \right.$$
            ...
            \begin{enumerate}[(US)]
                   \item Für jeden $R$-Modul $N$ und jede Familie $(f_i)_{i\in I}$ von $R$-Modulhom. $f_i:M_i\longrightarrow N$ existiert genau ein $R$-Modulhom. $f:M\mapsto N $ mit $f_i=f\circ q_i $ für alle $i\in J$
                   \[ 
                   \begin{xy}
                       \xymatrix{
                       M \ar@{-->}[rr]^{\exists!f}  &&N\\
        ...
        \begin{proof}
            analog zu 7.2
        \end{proof}
        \begin{definition}[dirkete Summe über univ. Eigenschaft]
        ...
        \end{definition}
        \begin{satz}[Universelle Eigenschaft freier Moduln]
            $M$ freier $R$-Modul mit Basis $(x_i)_{i\in I}.$ 
            Dann erfüllt das Tupel $(M,(x_i)_{i\in I})$ die Eigenschaft
            \begin{enumerate}[(UF)]
                   \item Für jeden $R$-Modul $N$ und jede Familie $(y_i)_{i\in I}$ aus $N$ existiert genau ein $R$-Modulhom. $f:M\longrightarrow N $ mit $f(x_i)=y_i $ für alle $i\in J,$
                   d.h. die Abbildung von Mengen 
                   \begin{align*}
        ...
        \end{notation}
        \begin{definition}
            Seien $L,M,N$ $R$-Moduln, $\varphi:M\times N\longrightarrow L.$
            \begin{align*}
                \varphi \text{ heißt $R$-bilinear }\Def& \text{ Für jedes $n\in N$ ist die Abbildung $M\longrightarrow L,m\mapsto \varphi(m,n)$}\\ 
                &\text{ ein $R$-Modulhom.}\\
                &\text{ und für jedes $m\in M$ ist die Abbildung $N\longrightarrow L,n\mapsto \varphi(m,n)$}\\
                &\text{ ein $R$-Modulhom.}\\
                &\text{ (d.h. $\varphi$ ist in jedem Argument bilinear)}
            \end{align*}
            Notation: $\Bil_R(M,N;L):=\{\varphi:M\times N\longrightarrow L| \varphi \text{ ist } R\text{-bilinear} \} $
        \end{definition}
        \begin{definition}
            Seien $M,N$ $R$-Moduln.\newline 
            Ein \textbf{Tensorprodukt} von $M$ und $N$ über $R$ ist ein $R$-Modul $T$ zusammen mit einer $R$-bilinearen Abbildung $\tau:M\times N \longrightarrow T$, sd. folgende universelle Eigenschaft erfüllt ist:
            \begin{enumerate}[(UT)]
                \item Für jeden $R$-Modul $L$ und jede $R$-bilineare Abbildung $\beta:M\times N\longrightarrow L$ existiert genau ein $R$-Modulhom. $f:T\longrightarrow L$, sd. $f\circ \tau=\beta$ ist.
                \[ \begin{xy}
                    \xymatrix{
                        T \ar@{-->}[rr]^{\exists!f} & &L\\
                        &\M\times N\ar[lu]^{\tau \text{ bil.}}\ar[ur]_{\beta \text{ bil.}}&\\
                    }
        
                \end{xy}]\]
                d.h. die Abbildung von Mengen 
                \begin{align*}
                    \Hom_R(T,L)&\longrightarrow \Bil_R(M,N;L)\\
        ...
        \end{definition}
        \begin{satz}
            Seien $M,N$ $R$-Moduln. Dann gilt
            \begin{enumerate}[(a)]
                \item Es gibt ein Tensorprodukt von $M,N$ über $R$.
                \item Sind $T,T'$ Tensorprodukte von $M,N$ über $R$ mit bilinearer Abbildung $\tau: M\times N\longrightarrow T,\tau':M\times N\longrightarrow T',$ dann existiert genau ein $R$-Modulisom. $\boldsymbol{\Phi}:T\longrightarrow T'$ mit $\boldsymbol{\Phi}\circ \tau =\tau'$
                \[ \begin{xy}
                    \xymatrix{
                        T \ar@{-->}[rr]^{\exists!\boldsymbol{\Phi}} & & L\\
                        &\M\times N\ar[lu]^{\tau }\ar[ur]_{\tau'}&
                    }
        
                \end{xy}]\]
                d.h.: Tensorprodukt von $M,N$ über $R$ ist eindeutig bestimmt bis auf eindeutige Isomorphie.
                \item Ist $T$ ein Tensorprodukt von $M,N$ über $R$ mit zugehöriger $R$-bil. Abb. $\tau:M\times N\longrightarrow T$ und setzen wir für $m\in M,n\in N$ 
                $$m\otimes n:=\tau(m,n),$$
                dann wird $T$ erzeugt von den Elementen $m\otimes n$ für $m\in M,n\in N$ (\textbf{reine Tensoren}). Insbesondere ist jedes Element aus $T$ von der Form $\sum\limits_{i=1}^{r}m_i\otimes n_i$ mit $m_i\in M,n_i\in N,$ d.h. ist Summe von reinen Tensoren. Hierbei gilt:
                \begin{align*}
                ...
                Notation: für Tensorprodukt von $M,N$ über $R$: $M\oplus_{R}N$
            \end{enumerate}
        \end{satz}
        \begin{proof}
            \begin{enumerate}[(a)]
                \item \begin{enumerate}[(1)]
                    \item 
                    \begin{align*}
                        \intertext{Es gibt eine injektive Abbildung}
                        i:M\times N &\hookrightarrow R^{(M\times N)}=\bigoplus_{k\in M\times N}R,(m,n)\longmapsto [m,n]=([m,n]_k)_{k\in M\times N}\\
                                    &\text{mit } [m,n]_{k}:=
                                    \left\{\begin{array}{cc}1, &\text{falls }k=(m,n)\\0, &\text{sonst}\end{array}\right.
                        \intertext{von Mengen (kein $R$-Modulhom.)}
                        \intertext{Die Elemente $[m,n]$ mit $m\in M, n\in N$ bilden eine Basis von $R^{(M\times N)},$ als $R$-Modul. Sei $U$ der Untermodul von $R^{(M\times N)},$ der von allen Elementen der folgenden Form erzeugt wird:}
                                [m+m',n]&-[m,n]-[m',n],\\
                                [m,n+n']&-[m,n]-[m,n'],\\
                                [rm,n]&-r[m,n],\\
                                [m,rn]&-r[m,n]\\
                        \intertext{Setze $T:= R^{(M\times N)}/U,\tau:M\times N\overset{i}{\hookrightarrow} R^{(M\times N)}\overset{\pi}{\underset{\text{kan. Proj.}}{\longrightarrow}} T=R^{(M\times N)}/U, (m,n)\longmapsto \overline{[m,n]}.$}
                        \intertext{Die Elemente von $\overline{[m,n]}$ mit $m\in M,n\in N$ bilden ein ES von $T$.}
                        \end{align*}
                        \item $T$ ist zusammen mit $\tau$ ein Tensorprodukt von $M,N$ über $R:$\newline
                        \begin{itemize}
                            \item $T$ ist $R$-Modul nach Konstruktion
                            \item $\tau$ ist $R$-bilinear:
                            $$\tau(m+m',n)=\overline{[m+m',n]}=\overline{[m,n]}+\overline{[m',n]}=\tau(m,n)+\tau(n',n)$$
                            andere Eigenschaften folgen analog (allesamt aus der Def. von $U$)
                            \item $T$ erfüllt zusammen mit $\tau$ universelle Eigenschaft:\newline
                            Existenz von $f$ wie in (UT):\newline
                            Sei $L$ $R$-Modul, $\beta:M\times N\longrightarrow L$ $R$-bil. Nach (UF) existiert $R$-Modulhom. $g:R^{(M\times N)}\longrightarrow L$ mit $g([m,n])=\beta(m,n)$ (denn: $R^{(M\times N)}$ ist frei mit Basis $([m,n])_{m\in M,n\in N}.)$
                            Es ist $g(U)=0,$ denn:
                            \begin{align*}
                                g([m+m',n]-[m,n]-[m',n])&\overset{g \text{ $R$-Modhom.}}{=}g([m+m',n])-g([m,n])-g([m'n])\\
                                &=\beta(m+m',n)-\beta(m,n)-\beta(m',n)\\
                                &\overset{\text{$\beta$ $R$-bil.}}{=}0
                            \end{align*}
                            Der Rest geht analog. 
                            \begin{align*}
                                &\implies \text{erhalte $R$-Modulhom.}
                                &f:T=R^{(M\times N)}/U\longrightarrow L \text{ mit }f(\underbrace{\overline{[m,n]}}_{=\tau(m,n)})=g([m,n])\\
                                &=\beta([m,n])\\
                                \intertext{(wohldef. wg. $g(U)=0$)}
                        ...
                        \end{itemize}
                    \end{enumerate}
                    \item mit den Standartargumenten, ähnlich wie in 7.2
                    \item Wegen (b) genügt es, Aussage (c) für $T$ aus der Konstruktion in Beweis von (a) nachzurechnen. Dieses wird erzeugt von den Elementen $\overline{[m,n]}=\tau(m,n)=m\otimes n,$ und es gilt 
                    $$(m+m')\otimes n=\tau(m+m',n)\tau(m,n)+\tau(m',n)=m\otimes n+m'\otimes n$$ analog für restliche Eigenschafen. Insbesondere ist $\sum\limits_{i=1}^{k}r_i(m_i\otimes n_i)=\sum\limits_{i=1}^{k}(rm_i)\otimes n_i,$ d.h. jedes Element aus $M\otimes_R N $ ist endliche Summe von reinen Tensoren.
            \end{enumerate}
        \end{proof}
        \begin{anm}
            \begin{itemize}
                \item Wir werden die Details der Konstruktion aus (a) im Folgenden nicht mehr brauchen, sondern nur (?)
                \item \begin{align*}
                    \text{die universelle Eigenschaft (UT): }&\text{ Für jeden $R$-Modul $L$ und jede $R$-bil. Abb. $\beta M\times N\longrightarrow L$ }\\
                    &\text{ existiert genau ein $R$-Modulhom. $f:M\otimes_R N\longrightarrow L$ mit}\\
                    &\text{$f(m\otimes n)\beta(m,n)$}\\
                    &\text{ und alle $m\in M,n\in N$ }
                \end{align*}
                \item die Aussage aus (c), d.h. Elemente aus $M\otimes_R N$ sind von der Form $\sum\limits_{i=1}^{k}m_i\otimes n_i,$ sind also endliche Summen reiner Tensoren mit Rechenregeln aus (c).
                \item Achtung: Im Allgemeinen ist nicht jedes Element von $M\otimes_R N$ ein reiner Tensor. 
                \item $M\otimes 0=0$ für alle $m\in M,$ denn: $m\otimes 0=m\otimes (0+0)=m\otimes 0+m\otimes 0$ Analog: $0\otimes n=0$ für alle $n\in N$
                \item Ist $(x_i)_{i\in I}$ ein ES von $M$ und $(y_i)_{i\in I}$ ein ES von $N,$ dann $(x_i\oplus y_j)_{(i,j)\in I\times J}$ ein ES von $M\oplus_R N.$
            \end{itemize}
        \end{anm}
        \begin{bsp}
            
            $\Z/2\Z\oplus_{\Z}\Z/3\Z=0$,
            denn:
            $$\text{Für } a\in \Z/2\Z,b\in \Z/3\Z \text{ ist }a\otimes b\underset{3\equiv 1(\mathrm{mod} 2)}{=} (3a)\otimes b=a\otimes (3b)\underset{3\equiv 0(\mathrm{mod} 3)}{=} a\otimes 0= 0$$
           
        \end{bsp}
        \begin{bem}
            Sei $M$ $R$-Modul.
            \begin{align*}
                \intertext{Dann ist die Abbildung}
                f&:R\otimes_R M\longrightarrow M, \sum\limits_{i=1}^{k}\underset{\in R}{a_i}\otimes \underset{\in M}m_i\longmapsto \sum\limits_{i=1}^{k}a_im_i\\
                \intertext{ist ein $R$-Modulisomorphismus mit Umkehrabbildung }
                g&:M\longrightarrow R\otimes_R M, m\longmapsto 1\otimes m,\\
                \intertext{d.h. $R\otimes_R M\cong M$}
                \intertext{Analog. $M\oplus_R R \cong M.$}
            \end{align*}
        \end{bem}
        \begin{proof}
            Die Abbildung $R\times M\longrightarrow M, (a,m)\longmapsto am$ ist $R$-bilinear. $\overset{\text{(UT)}}{\implies}$ Es existiert ein $R$-Modulhom. $f:R\otimes_R M\longrightarrow M$ mit $f(a\otimes m)=am$ (das ist das $f$ aus der Formulierung der Aussage). Die Abbildung $f:M\longrightarrow R\otimes_R M,m\longmapsto 1\otimes m$ ist ein $R$-Modulhom., denn:
            \begin{align*}
                g(m+n)&=1\otimes (m+n)=1\otimes m+1\otimes n=g(m)+g(n),\\
                g(rm)&= 1\otimes 1\otimes (rm)=r(1\otimes m)=rg(m)
            \end{align*}
            Es ist $(g\circ f)(a\otimes m)=g(am)=1\otimes(am)=(a\cdot 1)\oplus m=a\otimes m $ $\overset{(a\otimes m)\text{ ist}}{\implies}g\circ f= \id_{R\otimes_RM}.$ ES von $R\otimes_R M:$
            Es ist $(f\circ g)(m)=f(1\otimes m)=1m=m\implies f\circ g= \id_M\implies f,g$ sind invers zueinander.
        \end{proof}
        \begin{bsp}
            \begin{alignat*}{2}
            &\Z\oplus_{\Z}\Z/3\Z \cong \Z/3/Z &&\qquad\text{(aus 8.5)}
            \end{alignat*}
        \end{bsp}
        \begin{bem}
            Seien $M,N$ $R$-Moduln. Dann ist die Abbildung 
            $$f:M\times N\longrightarrow N\otimes_R M, \sum\limits_{i=1}^{k}m_i\otimes n_i\longmapsto \sum\limits_{i=1}^{k}n_i\otimes m_i$$ ein $R$-Modulisom., d.h. $M\oplus_R N\cong N\oplus_R M.$ 
        \end{bem}
        \begin{proof}
            Die Abbildung $\beta: M\times N\longrightarrow N\otimes_R N, (m,n)\longmapsto n\otimes m$ ist $R$-bilinear wegen Rechenregeln für Tensoren $\overset{\text{(UT)}}{\implies}$ Erhalte $R$-Modulhom. $f:M\otimes_R N\longrightarrow N\otimes_R M$ mit $f(m\otimes n)=n\otimes m$ für $m\in M,n\in N.$ Analog: Erhalte $R$-Modulhom. $g:N\otimes_R M\longrightarrow M\otimes_R N $ mit $g(n\otimes m)=m\otimes n$ für $n\in N, m\in M.$ $g,f$ sind invers zueinander.
        \end{proof}
        \begin{bem}
            $I\subseteq R$ Ideal, $M$ $R$-Modul. Dann ist die Abbildung 
            \begin{align*}
                f:R/I\otimes_R M&\longrightarrow M/IM\\
                \sum_{i=1}^{k}(r_i+I)\otimes m_i &\longmapsto \sum_{i=1}^{k}r_im_i+
            \end{align*}
            ist ein $R$-Modulisom. d.h. $R/I\oplus_R M\cong M/IM.$ 
        \end{bem}
        \begin{proof}
            \begin{enumerate}[1.]
                \item Die Abbildung $$\beta:R/I\times M\longrightarrow M/IM, (r+I,m)\longmapsto rm+IM$$ ist wohldefiniert und $R$-bilinear. $\implies$ Es existiert genau ein $R$-Modulhom. $f:R/I\otimes_R M\longrightarrow M/IM$ mit $f((r+I)\otimes m)=rm+IM$
                \item $f$ ist surjektiv, denn: $m+IM=f((1+I)\otimes m)$
                \item $f$ ist injektiv, denn: Sei $x\in \ker f$
                \begin{align*}
                    \text{Es ist }x&=\sum_{i=1}^{k}(r_i+I)\otimes m_i=\sum_{i=1}^{k}r_i((1+I)\otimes m_i)=(1+I)\otimes \underbrace{\sum_{i=1}^{k}r_im_i}_{:=m}\\
                    &\implies 0=f(x)=f((1+I)\otimes m)=m+IM\implies m\in IM\\
                    &\implies \text{ Es existiert }a_i\in I,\tilde{m_i}\in M \text{ mit }m=\sum_{i=1}^{r}a_i\tilde{m_i}\\
                    &\implies x=(1+I)\otimes m= (1+I)\otimes \sum_{i=1}^{r}a_i\tilde{m_i}\\
                    &=\sum_{i=1}^{r}(1+I)\otimes a_im_i=\sum_{i=1}^{r}(a_i+I)\otimes m_i=\sum_{i=1}^{r}(0+I)\otimes m_i\\
                    &= 0
                \end{align*}
            \end{enumerate}
        \end{proof}
        ...
        \end{Folgerung}
        \begin{proof}
            Wende 8.8 an auf $M=R/J.$ 
            \begin{align*}
            \implies R/I \otimes R/J&\cong (R/J)/(I(R/J))\\
            \text{Es ist } I(R/J)&=\{\sum\limits_{i=1}^{k}a_i(r_i+J)|a_i\in I,r_i\in R\}\\
            &=\{\sum\limits_{i=1}^{k}(a_ir_i+J)|a_i\in I,r_i\in R\}\\
            &=(I+J)/J\\
            &\implies R/I\otimes R/J\cong (R/J)/((I+J)/J)\\
            &\overset{6.10}{=}R/(I+J)
            \end{align*}
        \end{proof}
        \begin{bsp}
            $\Z/m\Z\otimes \Z/n\Z\cong \Z/(\ggT(m,n)\Z)$ (aus 8.9)
            $$\text{Es gilt: }\ggT(n,n)=1\implies \Z/m\Z\otimes_{\Z}\Z/n\Z=0, \text{ sowie }\Z/n/Z\otimes_{\Z}\Z/n\Z\cong \Z/n\Z.$$
        \end{bsp}
        \begin{bem}
            Seien $M,M',N,N'$ $R$-Modulhom. $:M\longrightarrow M', g:N\longrightarrow N'$ $R$-Modulhom. Dann gilt 
            \begin{enumerate}[(a)]
                \item Es gibt genau einen $R$-Modulhom. $f\otimes g: M\otimes N\longrightarrow M'\otimes N; $ mit $(f\otimes g)(m\otimes n)=f(m)\otimes g(n)$ für alle $n\in N.$
                \item $f,g$ Isomorphismen $\implies$ $f\otimes$ surjektiv
            \end{enumerate}
            Insbesondere gilt: $M\cong M'$ und $N\cong N'\implies M\otimes_R N\cong M'\otimes_R N'.$
        \end{bem}
        \begin{proof}
            \begin{enumerate}[(a)]
                \item Die Abb $\beta:M\times M\longrightarrow M'\otimes_R N',(m,n)\longmapsto f(n)\otimes g(n)$ ist $R$-bilinear $\implies$ Behauptung folgt aus (UT).
                \item $f^{-1}\otimes g^{-1}$ ist invers zu $f\otimes g$
                \item Sei $z=\sum\limits_{i=1}^{k}m_i'\otimes n_i'$ mit $m_i'\in M',n_i\in N'$\newline 
                Seien $m_i\in M$ mit $f(m_i)=m_i',n_i\in N $ mit $g(n_i)=n_i'$ (ex. weil $f,g$ surjektiv) $\implies (f\otimes g)(\sum\limits_{i=1}^{k}m_i\otimes n_i)=\sum\limits_{i=1}^{k}f(m_i)\otimes f_{n_i}=\sum\limits_{i=1}^{k}m_i'\otimes n_i'=z$
            \end{enumerate}
        \end{proof}
        \begin{bsp}
        Sind $f,g$ injektiv, dann braucht $f\otimes g$ nicht injektiv sein: Setze 
        \begin{align*}
        f&:\Z\longrightarrow\Z,x\longmapsto 2x\\
        g=\id_{\Z/2\Z}&:\Z/2\Z\longrightarrow \Z/2\Z
        \end{align*}
        (beide injektiv)
        Aber: Für $f\otimes g: \Z\otimes_{\Z}\Z/2\Z\longrightarrow \Z\otimes_{\Z}\Z/2\Z$ gilt:
        \begin{align*}
            (f\otimes g)(x\otimes y) &=f(x)\otimes g(y)=2x\otimes y=x\otimes 2y\\
            &=x\otimes 0\\
            &=0\\
            &\implies f\otimes g \text{ ist Nullabbildung und }\Z\otimes_{\Z}\Z/2\Z\neq 0,\\
            &\text{ also ist $f\otimes g$ nicht injektiv.}
        \end{align*}
        \end{bsp}
\end{document}